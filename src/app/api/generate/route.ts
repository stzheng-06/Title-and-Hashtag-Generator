...
请根据以上信息生成:
1. 5-10个吸引人的标题（注意：标题中不要包含#号，不要包含任何标签符号，只要纯标题文字）
...
const buildTitlesPrompt = (keywords: string[], config: Config): string => {
  const enabledRules = config.generationRules
    .filter(rule => rule.enabled)
    .map(rule => `- ${rule.description}`)
    .join('\n')
  ...
  return `你是一个专业的内容创作助手。请根据以下信息生成吸引人的标题。
...
请生成5-10个吸引人的标题。注意：标题中不要包含#号，不要包含任何标签符号，只要纯标题文字。
...
const buildTagsPrompt = (keywords: string[], config: Config): string => {
  const enabledRules = config.generationRules
    .filter(rule => rule.enabled)
    .map(rule => `- ${rule.description}`)
    .join('\n')
  ...
  return `你是一个专业的内容创作助手。请根据以下信息生成相关的主题标签。
...
const parseAIResponse = (content: string): { titles: string[], tags: string[] } => {
  try {
    ...
    if (typeof content !== 'string' || !content) return { titles: [], tags: [] }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { keywords, config, generationType } = body as GenerateRequest & { generationType?: 'titles' | 'tags' }
    ...
    // 构建 prompt
    let prompt: string
    if (fullConfig.customPrompt?.trim()) {
      prompt = fullConfig.customPrompt.replace(/\{keywords\}/g, keywords.join(', '))
    } else if (generationType === 'titles') {
      prompt = buildTitlesPrompt(keywords, fullConfig)
    } else if (generationType === 'tags') {
      prompt = buildTagsPrompt(keywords, fullConfig)
    } else {
      prompt = buildPrompt(keywords, fullConfig)
    }
    ...
    const content = Array.isArray(rawContent) ? rawContent.map((x: any) => typeof x === 'string' ? x : (x?.text ?? '')).join('\n') : String(rawContent ?? '')
